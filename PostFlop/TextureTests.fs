...
let ``toFlopOptions when FD`` () =
  ...
  let expected = { Options.CbetFactor = Always 50m; CheckRaise = OnCheckRaise.CallEQ 1; Donk = OnDonk.ForValueStackOff }
  Assert.Equal(expected, actual)
...
let ``toTurnOptions Cbet not FV card`` () =
  ...
  let expected = { Options.CbetFactor = Never; CheckRaise = OnCheckRaise.Undefined; Donk = OnDonk.Undefined }
  Assert.Equal(expected, actual)
